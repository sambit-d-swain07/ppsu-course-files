...
// 1. Update pdf-generator.ts
const pdfGenPath = path.join(__dirname, '..', 'src', 'lib', 'pdf-generator.ts');

const pdfGenCode = `import fs from 'fs';
...
fs.writeFileSync(pdfGenPath, pdfGenCode, 'utf8');

// 2. Update preview/page.tsx button to point to /api/course-files/${courseFileId}/merged-pdf
const previewPath = path.join(__dirname, '..', 'src', 'app', 'faculty', 'course-files', '[id]', 'preview', 'page.tsx');
...
fs.writeFileSync(previewPath, previewCode, 'utf8');
console.log('Successfully updated pdf-generator.ts and preview/page.tsx!');
